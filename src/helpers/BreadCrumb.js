            ...
            <div class="d-flex" style={{alignItems: 'center'}}>
                {
                    paths.map((path, index) => {
                        ...
                        return path === ""
                            ? <span class="d-flex"><Link to="/"><i className="pi pi-home" style={{fontSize: '18px', color: '#000'}}></i></Link> {' '}{(index + 1) <= size ? <span style={{fontSize: '15px', marginLeft: '10px', marginRight: '10px'}}>/</span> : ""}</span>
                            : <span>{' '}
