...
const POIEdit = ({ POIArray, componentStatus, closeEditMode }) => {
  ...
  const [POIs, setPOIs] = useState(POIArray);
        ...
        <div className="p-card p-mt-2">
          ...
          <div className="p-card-body">
            <form onSubmit={handleSubmit(POISubmit)}>
              ...
              {POIs?.map((POI, index) => (
                <button key={index} onClick={(e) => handlePOIDelete(e)} type="button" className="p-mr-2 tag-container" id={POI.id}><Tag value={POI.POIName} icon="pi pi-times" className="p-p-2" ></Tag>
                </button>
              ))}
