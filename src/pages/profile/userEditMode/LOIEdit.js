...
const LOIEdit = ({ LOIArray, componentStatus, closeEditMode }) => {
  ...
  const [LOIs, setLOIs] = useState(LOIArray);
    ...
    <>
      {componentStatus.LOIEdit &&
        <div className="p-card p-mt-2">
          <SectionHeader componentStatus={componentStatus} icon="briefcase" sectionTitle="Locations of Interest" />
          <div className="p-card-body">
            <form onSubmit={handleSubmit(LOISubmit)}>
              ...
              {LOIs?.map((LOI, index) => (
                <button key={index} onClick={(e) => handleLOIDelete(e)} type="button" className="p-mr-2 tag-container" id={LOI.id}><Tag value={LOI.LOIName} icon="pi pi-times" className="p-p-2" ></Tag>
                </button>
              ))}
