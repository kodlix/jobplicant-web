...
const PortfolioEdit = ({ componentStatus, portfolioFromBack, closeEditMode }) => {
  const { handleSubmit } = useForm();
  const toast = useRef(null);
  const ref = useRef({});
  const [portfolioItems, setPortfolioItems] = useState([]);
  useEffect(() => {
    if (componentStatus?.portfolioEdit?.length > 0) {
      if (componentStatus?.portfolioEdit !== "add") {
        setPortfolioItems(portfolioFromBack)
      }
    }
  ...
  return (
    <>
      {componentStatus?.portfolioEdit?.length > 0 &&
        <div className="p-card p-mt-2">
          ...
          <div className="p-card-body">
            <form onSubmit={handleSubmit(portfolioSubmit)}>
              ...
              <EditModeFooter id="portfolioEdit" onCancel={onEditCancel} />
            </form>
          </div>
        </div>
      }
